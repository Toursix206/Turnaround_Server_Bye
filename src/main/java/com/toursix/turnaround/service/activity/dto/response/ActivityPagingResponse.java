...
public class ActivityPagingResponse {
    ...
    public static ActivityPagingResponse of(Page<Activity> activityPaging) {
        if (!activityPaging.hasNext()) {
            return ActivityPagingResponse.newLastScroll(activityPaging.getContent(), activityPaging.getTotalPages() - 1);
        }
        return ActivityPagingResponse.newPagingHasNext(activityPaging.getContent(), activityPaging.getTotalPages() - 1, activityPaging.getPageable().getPageNumber() + 1);
    }
    ...
    private static ActivityPagingResponse newPagingHasNext(List<Activity> activityPaging, long lastPage, long nextPage) {
